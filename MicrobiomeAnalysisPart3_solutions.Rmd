---
title: "MicrobiomeAnalysisPart3_Phyloseq"
...
knitr::opts_knit$set(root.dir = "/Users/rebeccavalls/Dartmouth College Dropbox/Rebecca Valls/FAIR2024/Valls_Hilliam/MicrobiomeDay2/MicrobiomeAnalysisPart3") #This line sets the root directory for the R Markdown file. 
```

## References: 

https://joey711.github.io/phyloseq/ #more detailed phyloseq tutorial
https://www.nature.com/articles/ismej2017119 #difference between ASVs and OTUs
...
-ASV counts (July20_2024_MicrobiomeAnalysisPart2_counts.tsv)
-taxonomy of each ASV (July20_2024_MicrobiomeAnalysisPart2_taxa.tsv)
...
We calculate absolute abundance of a single genus, Bacteroides.

We identify significantly differentially abundant ASVs across our dataset
...
```{r}
#You need to replace the path with your own directory. You can do this by navigating through Files Pane to your directory, and clicking "Set as Working Directory". Then copy the path to the code below.
count_df <- as.data.frame(read.table("~/Dropbox (Dartmouth College)/FAIR2024/Valls_Hilliam/MicrobiomeDay1/MicrobiomeAnalysisPart2/2_DADA2Output/July20_2024_MicrobiomeAnalysisPart2_counts.tsv", header=T, row.names=1, check.names=F, sep="\t"))
...
tax_df <- as.matrix(read.table("July20_2024_MicrobiomeAnalysisPart2_taxa.tsv", header=T, row.names=1, check.names=F, sep="\t"))
...
filteredASVs = genefilter_sample(Phyloseq_subset, #genefilter_sample() function is used to filter samples based on certain criteria
                                 ...
                                 A=0.1*nsamples(Phyloseq_subset)) #A argument specifies the minimum proportion of samples in which an ASV must exceed the filtering threshold (1 in this case). An ASV must exceed the threshold (abundance > 1) in at least 10% of the samples to be retained.
...
# Run PERMANOVA using adonis2
before_LPS_perm_results <- adonis2(distance_bray_Before_LPS ~ Treatment + Sex, data = ord_Before_LPS_df, permutations = 999)
...
ggplot(dat.dataframe.agr, aes(x = Treatment, y = Abundance, fill = Genus)) +
...
## Multivariable associate with MaAsLin
```{r}
wd <- c("~/Dartmouth College Dropbox/Rebecca Valls/FAIR2024/Valls_Hilliam/MicrobiomeDay2/MicrobiomeAnalysisPart3")

genus_rel_abundance_df %>%
  filter(Treatment != "Baseline") %>%
  dplyr::select(Sample,OTU,Abundance) %>%
  pivot_wider(names_from = OTU,
              values_from = Abundance,
              values_fn = mean) -> counts

column_to_rownames(counts, 
                   var = "Sample") -> counts

genus_rel_abundance_df %>%
  filter(Treatment != "Baseline") %>%
  dplyr::select(Sample,Cage,Mouse_.,Treatment) %>%
  distinct() -> meta

meta %>%
  mutate(cage.mouse = paste(Cage,Mouse_.,
                            sep = ".")) %>%
  dplyr::select(Sample,cage.mouse,Treatment) -> meta
...
Maaslin2(input_data = counts,
         ...
         random_effects = c("Cage","Mouse_."))

# Analysis tells us ASV_21 is the only significantly different ASV - let's look up which taxa this equates to

tax_df["ASV_21",] # ASVs are assigned as row names so we can use base R to extract the row called ASV_21
...
#Do not filter out any ASVS, but transform to relative abundance 
Phyloseq_LPS_transformed = transform_sample_counts(Phyloseq_LPS, function(x) x/sum(x)*100) 
...
total_gene_copies_noAllobacillus <- total_gene_copies - ((20/20) * 1.4*10^8)
total_gene_copies_noAllobacillus_noImtechella <- total_gene_copies_noAllobacillus-Imtechella_theoretical
...
#Use genus_Phyloseq_LPS_no_spikein_transformed
...
#Green
#1. What is the purpose of the ggtitle() function in the script?
#Answer: The ggtitle() function is used to add a title to the plot generated by ggplot2.

#2. What is the purpose of using transform_sample_counts() on the pruned Phyloseq object?
#Answer: The purpose of using transform_sample_counts() on the pruned Phyloseq object is to normalize the counts of ASVs across samples, by expressing them as relative abundances (percentages).

#3. What is the purpose of the phyla_counts_tab object?
#Answer: The phyla_counts_tab object calculates the number of counts for each phylum in the OTU table of the pruned Phyloseq object.

#Blue
#1. What does the measures argument in the plot_richness() function do?
#Answer: The measures argument specifies the alpha diversity measures to be plotted. In this case, it is set to "Shannon" and "Chao1".

#2. What does the Bray-Curtis dissimilarity distance matrix represent?
Answer: The Bray-Curtis dissimilarity distance matrix represents the dissimilarity between pairs of samples based on their microbial community composition. It is calculated based on the abundance of different microbial taxa in each sample, and it ranges from 0 (identical communities) to 1 (completely dissimilar communities).

#3. What is the purpose of the merge() function used in the script?
Answer: The merge() function is used to merge the narrow-format data frame created from the phyla_counts_tab object with a metadata table containing additional information about each sample.

#4. Create variable importance plots of the sex random forest model with genus names and save them as pdfs
# Extract variable importance
var_importance_sex_rf <- importance(sex_rf)

# Convert to data frame for easier manipulation
var_importance_sex_rf_df <- as.data.frame(var_importance_sex_rf)

# Add a column for variable names
var_importance_sex_rf_df$Variable <- rownames(var_importance_sex_rf_df)

# Sort by the importance measure (MeanDecreaseGini or MeanDecreaseAccuracy)
# Assuming you want to use MeanDecreaseGini
var_importance_sex_rf_df <- var_importance_sex_rf_df[order(var_importance_sex_rf_df$MeanDecreaseGini, decreasing = TRUE),]

# Select the top 10 variables
top_10_vars_sex <- head(var_importance_sex_rf_df, 10)

# Print the top 10 variables
print(top_10_vars_sex)

# Convert row names to a column in top_10_vars (if they are not already)
genus_names_df$Variable <- rownames(genus_names_df)

# Add the 'Genus' column to top_10_vars by merging with genus_names_df
top_10_vars_sex_with_genus <- top_10_vars_sex %>%
  left_join(genus_names_df, by = "Variable")
...
top_10_vars_sex_with_genus <- top_10_vars_sex_with_genus %>%
  arrange(desc(MeanDecreaseGini))

# Create the plot2
MeanDecreaseAccuracy_sex <- ggplot(top_10_vars_sex_with_genus, aes(x = MeanDecreaseAccuracy, y = reorder(Genus, MeanDecreaseAccuracy))) +
  geom_col(fill = "skyblue") +
  theme_minimal() +
  labs(
    x = "Mean Decrease Accuracy",
    y = "Genus",
    title = "Mean Decrease Accuracy of Sex RF"
  )+
  theme(
    axis.text.y = element_text(face = "italic")
  )
...
#Black
#1. How would you modify the script to plot the Observed AVSs alpha diversity index of the cecum of mice at baseline ?
#Hint: Cec indicates that sample was collected from the Cecum of that mouse 


#2. What is the rationale behind removing ASVs that appear less than one time in more than 10% of the samples?
Answer: The rationale behind removing ASVs that appear less than one time in more than 10% of the samples is to reduce the noise in the dataset and improve the accuracy of downstream analysis. ASVs with low counts are likely to be noise or sequencing errors, and filtering them out reduces the number of false positives in the dataset.
